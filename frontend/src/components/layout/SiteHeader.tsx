...
export function SiteHeader({ className, onNavigate, showMarketingLinks = true, showPingRail = true }: Props) {
  ...
  return (
    <header className={cn("sticky top-0 z-40 border-b border-border console-chrome", className)}>
      ...
      {showPingRail && (
        <div className="section-wrap border-t border-border py-1.5">
          <LivePingRail />
        </div>
      )}
            ...
            {NAV_LINKS.map((link) =>
              link.external ? (
                <a
                  ...
                  target="_blank"
                  rel="noopener"
                  className="border-b border-border px-1 py-3 text-sm font-medium hover:text-accent"
                  onClick={closeMenu}
                >
                  {link.label}
                </a>

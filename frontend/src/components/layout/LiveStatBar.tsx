      ...
      {loading
        ...
        : probes.map((r) => (
            <Badge
              ...
              className="font-mono text-[11px] glass backdrop-blur-sm"
            >
              {r.region_id} · {r.latency_ms != null ? `${Math.round(r.latency_ms)}ms` : "—"}
            </Badge>
          ))}
      {syncInterval != null && (
        <Badge variant="accent" className="font-mono text-[11px] glass">
          sync {syncInterval}s
        </Badge>
      )}
      {knowledgeDocs != null && (
        <Badge variant="default" className="font-mono text-[11px] glass">
          {knowledgeDocs} docs
        </Badge>
      )}
    </div>

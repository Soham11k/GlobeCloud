...
stock_before=$(echo "$products_json" | python3 -c "import sys,json; ps=json.load(sys.stdin)['products']; p=next(x for x in ps if x['id']=='prod-001'); print(p['stock'])")
...
stock_after_order=$(echo "$products_after" | python3 -c "import sys,json; ps=json.load(sys.stdin)['products']; p=next(x for x in ps if x['id']=='prod-001'); print(p['stock'])")
...
stock_sync1=$(echo "$products_sync1" | python3 -c "import sys,json; ps=json.load(sys.stdin)['products']; p=next(x for x in ps if x['id']=='prod-001'); print(p['stock'])")

curl_json -X POST "${API}/sync/run" >/dev/null
sleep 1
products_sync2=$(curl_json "${API}/regions/${region}/products")
stock_sync2=$(echo "$products_sync2" | python3 -c "import sys,json; ps=json.load(sys.stdin)['products']; p=next(x for x in ps if x['id']=='prod-001'); print(p['stock'])")
